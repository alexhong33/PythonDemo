...
import time
import urllib.error
from DemoUrllib.Demo02基本添加报头方法一 import headers
...
def getcontent(listurl, proxy):
    ...
    fh = open("F:/SpiderWeChat/1.html", "wb")
    ...
    fh = open("F:/SpiderWeChat/1.html", "wb")  
    #此时lisurl为二维列表, 形如listurl[][], 第一维存储的信息跟第几页相关, 第二维存储的跟该页第几个文章链接相关
    for i in range(0, len(listurl)):
        for j in range(0, len(listurl[i])):
            try:
                url = listurl[i][j]
                
                #处理成真实url, 读者亦可以观察对应网址的关系自行分析, 采集网址比真实网址读了一串amp
                url = url.replace("amp;", "")
                #使用代理去爬取对应网址的内容
                ...
                title = re.compile(contentpat, re.S).findall(data)
                ...
                #如果标题列表不为空, 说明找到了标题, 取列表第0个元素, 即此次标题赋给列表thistitle
                if(title != []):
                    thistitle = title[0]
                if(content != []):
    ...
    '''
    fh = open("F:/SpiderWeChat/1.html", "ab")
...
key = "物联网"   
#设置代理服务器, 该代理服务器有可能失效
proxy = "218.18.232.29:8080"
#可以为getlisturl()与getcontent()设置不同的代理服务器, 此处没有启动该项设置
proxy2 = ""
#起始页面
pagestart = 1
#爬取到哪页
